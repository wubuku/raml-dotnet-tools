...
The API architect MAY use any versioning scheme so long as version numbers retain the same format. For example, "v3", "v3.0", and "V3" are all allowed, but are not considered to be equal.`)]
        ...
        ),MetaModel.issue("Duplicate of base uri parameters"),MetaModel.issue("https://github.com/raml-org/raml-spec/issues/61")]
...
`),MetaModel.needsClarification("should be open enum"),MetaModel.inherited()]
        ...
        $securedBy=[MetaModel.issue("Not stated clearly in spec"),MetaModel.description(`A list of the security schemas to apply to all methods, these must be defined in the securitySchemes declaration.`)]
